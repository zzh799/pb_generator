...
public class MessageLineMarkerProvider implements LineMarkerProvider {
    ...
    private static final GutterIconNavigationHandler<PbEnumDefinitionImpl> SHOW_Enum_SENDERS =
            (e, psiElement) -> {
                if (psiElement != null) {
                    Project project = psiElement.getProject();
                    String projectPathUri = project.getBasePath();
    ...
    public LineMarkerInfo<?> getLineMarkerInfo(@NotNull PsiElement psiElement) {
        ...
        } else if (psiElement instanceof PbEnumDefinitionImpl) {
            return createEnumLineMarkerInfo((PbEnumDefinitionImpl) psiElement);
        }
        return null;
    }

    private LineMarkerInfo<?> createEnumLineMarkerInfo(PbEnumDefinitionImpl element) {
        return new LineMarkerInfo<>(element,
                element.getTextRange(),
                MyIcons.protoMessage,
                e -> "generate " + element.getName(),
